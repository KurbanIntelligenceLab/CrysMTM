...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    def forward(self, f_in, pos, batch, node_atom, **kwargs) -> torch.Tensor:
        # Dynamically map atomic numbers to contiguous indices
        unique_atoms, node_atom_mapped = torch.unique(node_atom, sorted=True, return_inverse=True)
        max_atom_type = unique_atoms.numel()
        # Create embedding network on the fly for the current batch
        atom_embed = NodeEmbeddingNetwork(self.irreps_node_embedding, max_atom_type)
        ...
        # Use mapped node_atom for embedding
        atom_embedding, atom_attr, atom_onehot = atom_embed(node_atom_mapped)
