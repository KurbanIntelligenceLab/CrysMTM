...
def add_node_atom(batch):
    # Add node_atom attribute (atomic numbers) if not present
    if not hasattr(batch, 'node_atom'):
        ...
        if hasattr(batch, 'element'):
            ...
            raise ValueError("Cannot infer node_atom (atomic numbers) for Equiformer input.")
    return batch
    ...
    for target_index, target_name in enumerate(TARGET_NAMES):
        ...
        for seed in SEEDS:
            ...
            out_dir = os.path.join(
                "results", "regression", f"equiformer_{target_name}", str(seed)
            )
