...
from models.regression.clip_regressor import (
    DEFAULT_MODEL_NAME,
    create_clip_regressor,
)
...
def collate_fn_clip(batch, processor):
    ...
    return {**inputs, "regression_label": regression_labels}
    ...
    for target_index, target_name in enumerate(TARGET_NAMES):
        ...
        for seed in SEEDS:
            ...
            out_dir = os.path.join(
                "results", "regression", f"clip_{target_name}", str(seed)
            )
            ...
            optimizer = optim.AdamW(model.regressor.parameters(), lr=LEARNING_RATE)
