    ...
    for target_index, target_name in enumerate(TARGET_PROPERTIES):
        ...
        for seed in SEEDS:
            ...
            out_dir = os.path.join(
                "results", "regression", f"egnn/{target_name}", str(seed)
            )
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                val_loss, val_mae = evaluate(
                    model, reg_head, val_loader, device, desc=f"Epoch {epoch+1}/{NUM_EPOCHS} [Val]", target_index=target_index, normalizer=normalizer
                )
                print(
