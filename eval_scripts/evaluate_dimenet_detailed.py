...
from models.regression.dimenet_regressor import DimeNetModel

TEST_TEMPS = ID_TEMPS + OOD_TEMPS


def load_dimenet_model(target_name, seed, device):
    """Load trained DimeNet++ model."""
    model_path = os.path.join(
        "results", f"regression/dimenetpp/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/dimenetpp/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    model = DimeNetModel.to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))

    return model, normalizer


def evaluate_dimenet_detailed(model_info, dataloader, device, target_name, seed):
    """Evaluate DimeNet++ model and return detailed predictions with metadata."""
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # DimeNet++ specific logic - uses z, pos, batch
            ...
            for i in range(batch_size):
                ...
                if sample_idx < len(dataloader.dataset):
                    ...
                    try:
                        ...
                        elif hasattr(sample, "metadata") and hasattr(
                            sample.metadata, "temperature"
                        ):
                            temperature = sample.metadata.temperature
                        elif hasattr(dataloader.dataset, "data") and sample_idx < len(
                        ...
                        composition = None
                        print(
                            f"WARNING: Error extracting metadata at batch {batch_idx}, sample {i}: {e}"
                        )

                    if temperature is None or composition is None:
                        print(
                            f"WARNING: Missing metadata at batch {batch_idx}, sample {i}"
                        )
...
def main():
    ...
    combined_path = os.path.join(output_dir, "dimenetpp_predictions.csv")
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                model_path = os.path.join(
                    "results",
                    f"regression/dimenetpp/{target_name}",
                    str(seed),
                    "best_model.pth",
                )
                ...
                model_info = load_dimenet_model(target_name, seed, device)
                model, normalizer = model_info

                # Use appropriate dataloader logic for DimeNet++ (PyG data)
                ...
                predictions, targets, metadata = evaluate_dimenet_detailed(
                ...
                first_write = False
                print(
                    f"Saved results for regression/dimenetpp/{target_name}/{seed} to {combined_path}"
                )

            except Exception as e:
                print(
                    f"Error evaluating regression/dimenetpp/{target_name}/{seed}: {e}"
                )
                continue

    print(f"All DimeNet++ results saved to {combined_path}")
