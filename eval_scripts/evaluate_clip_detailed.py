...
def load_clip_model(target_name, seed, device):
    ...
    model = create_clip_regressor(model_name=DEFAULT_MODEL_NAME, freeze_backbone=True).to(device)
...
def collate_fn_clip(batch, processor):
    ...
    result = {**inputs, "regression_label": regression_labels, "temperatures": temperatures, "phases": phases}
    return result
        ...
        for seed in SEEDS:
            try:
                ...
                dataloader = DataLoader(
                    ...
                    num_workers=0, collate_fn=lambda b: collate_fn_clip(b, processor)
                )
