...
def load_schnet_model(target_name, seed, device):
    """Load trained SchNet model."""
    model_path = os.path.join(
        "results", f"regression/schnet/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/schnet/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
...
def evaluate_schnet_detailed(model_info, dataloader, device, target_name, seed):
    """Evaluate SchNet model and return detailed predictions with metadata."""
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # SchNet specific logic - uses z, pos, batch
...
def main():
    ...
    combined_path = os.path.join(output_dir, "schnet_predictions.csv")
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                model_path = os.path.join(
                    "results",
                    f"regression/schnet/{target_name}",
                    str(seed),
                    "best_model.pth",
                )
                ...
                model_info = load_schnet_model(target_name, seed, device)
                model, normalizer = model_info

                # Use appropriate dataloader logic for SchNet (PyG data)
                ...
                predictions, targets, metadata = evaluate_schnet_detailed(
                ...
                first_write = False
                print(
                    f"Saved results for regression/schnet/{target_name}/{seed} to {combined_path}"
                )

            except Exception as e:
                print(f"Error evaluating regression/schnet/{target_name}/{seed}: {e}")
                continue

    print(f"All SchNet results saved to {combined_path}")
