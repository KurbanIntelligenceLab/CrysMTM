...
def load_visnet_model(target_name, seed, device):
    ...
    model_path = os.path.join(
        "results", f"regression/visnet/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/visnet/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                model_path = os.path.join(
                    "results",
                    f"regression/visnet/{target_name}",
                    str(seed),
                    "best_model.pth",
                )
                ...
                first_write = False
                print(
                    f"Saved results for regression/visnet/{target_name}/{seed} to {combined_path}"
                )
