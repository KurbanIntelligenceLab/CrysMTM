...
def load_pure2dopenet_model(target_name, seed, device):
    ...
    model_path = os.path.join(
        "results", f"regression/pure2dopenet/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/pure2dopenet/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                model_path = os.path.join(
                    "results",
                    f"regression/pure2dopenet/{target_name}",
                    str(seed),
                    "best_model.pth",
                )
                ...
                dataloader = DataLoader(
                    ...
                    shuffle=False,
                    num_workers=0,
                    collate_fn=collate_fn_pure2dopenet,
                )
                ...
                first_write = False
                print(
                    f"Saved results for regression/pure2dopenet/{target_name}/{seed} to {combined_path}"
                )

            except Exception as e:
                print(
                    f"Error evaluating regression/pure2dopenet/{target_name}/{seed}: {e}"
                )
                continue
