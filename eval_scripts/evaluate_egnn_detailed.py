...
def load_egnn_model(target_name, seed, device):
    ...
    model_path = os.path.join(
        "results", f"egnn/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"egnn/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    reg_head_path = os.path.join(
        "results", f"egnn/{target_name}", str(seed), "best_reg_head.pth"
    )
