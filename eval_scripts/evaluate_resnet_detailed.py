...
from models.regression.resnet_regressor import (
    DEFAULT_IMAGE_SIZE,
    DEFAULT_MODEL_NAME,
    create_resnet_regressor,
)
...
def load_resnet_model(target_name, seed, device):
    ...
    model_path = os.path.join(
        "results", f"regression/resnet/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/resnet/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    model = create_resnet_regressor(model_name=DEFAULT_MODEL_NAME, pretrained=False).to(
        device
    )
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                model_path = os.path.join(
                    "results",
                    f"regression/resnet/{target_name}",
                    str(seed),
                    "best_model.pth",
                )
                ...
                first_write = False
                print(
                    f"Saved results for regression/resnet/{target_name}/{seed} to {combined_path}"
                )
