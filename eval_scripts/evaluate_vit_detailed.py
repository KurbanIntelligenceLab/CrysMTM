...
def load_vit_model(target_name, seed, device):
    ...
    model_path = os.path.join(
        "results", f"regression/vit/{target_name}", str(seed), "best_model.pth"
    )

    # Load normalizer if available
    normalizer_path = os.path.join(
        "results", f"regression/vit/{target_name}", str(seed), "normalizer.pkl"
    )
    normalizer = None
    ...
    model = create_vit_regressor(model_name=DEFAULT_MODEL_NAME, pretrained=False).to(
        device
    )
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                ...
                first_write = False
                print(
                    f"Saved results for regression/vit/{target_name}/{seed} to {combined_path}"
                )
