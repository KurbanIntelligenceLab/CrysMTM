...
def load_vit_model(target_name, seed, device):
    ...
    model_path = os.path.join("results", f"vit/{target_name}", str(seed), "best_model.pth")
    
    # Load normalizer if available
    normalizer_path = os.path.join("results", f"vit/{target_name}", str(seed), "normalizer.pkl")
    normalizer = None
    ...
    for target_name in TARGET_PROPERTIES:
        for seed in SEEDS:
            try:
                ...
                first_write = False
                print(f"Saved results for vit/{target_name}/{seed} to {combined_path}")
                
            except Exception as e:
                print(f"Error evaluating vit/{target_name}/{seed}: {e}")
                continue
